...
Validates employee data before any payslip generation or email sending.
...
class ValidationError:
    ...
    def __repr__(self):
        return f"ValidationError(row={self.row}, field='{self.field}', message='{self.message}')"
...
class DataValidator:
    ...
    def validate_all(self) -> Tuple[List[ValidationError], List[ValidationError]]:
        ...
        Returns:
            Tuple of (errors, warnings). If errors is non-empty, processing
            should not proceed.
        """
        self.errors = []
        self.warnings = []
        ...
        if not self.employees:
            self.errors.append(
                ValidationError(0, "data", "No employee data found")
            )
        ...
        if not self.errors:
            logger.info(
                f"Validation passed: {len(self.employees)} employees OK"
            )
    ...
    def _validate_email_format(self):
        ...
        for emp in self.employees:
            ...
            if email and not EMAIL_PATTERN.match(email):
                self.errors.append(
                    ValidationError(
                        row, "Email", f"Invalid email format: '{email}'"
                    )
                )
    ...
    def _validate_duplicate_emails(self):
        ...
        for email, count in email_counts.items():
            if count > 1:
                # Find which rows have this duplicate
                rows = [
                    emp.get("row", 0)
                    for emp in self.employees
    ...
    def _validate_passwords(self):
        ...
        for emp in self.employees:
            ...
            if not password:
                self.errors.append(
                    ValidationError(
                        row, "Password", "Password is empty"
                    )
                )
